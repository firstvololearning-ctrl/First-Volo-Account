(function () {
  ...
  async function getAccountSnapshot() {
    ...
    const [profile, entitlements, classes, students, memberships, classProductAccess, adminStatus] = await Promise.all([
      ...
      client.rpc("get_entitlement_admin_status")
    ]);
    const queryError = profile.error || entitlements.error || classes.error || students.error || memberships.error || classProductAccess.error || adminStatus.error;
    if (queryError) {
    ...
    return { user, educatorProfile: profile.data || null, entitlements: entitlements.data.filter(row => Date.parse(row.starts_at) <= now && Date.parse(row.expires_at) > now), classes: classes.data, students: studentRows, classProductAccess: classProductAccess.data, isEntitlementAdmin: adminStatus.data === true, error: null };
  }
