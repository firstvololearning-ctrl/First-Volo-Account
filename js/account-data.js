(function () {
  ...
  async function getAccountSnapshot() {
    ...
    if (!client) return { user: null, educatorProfile: null, entitlements: [], classes: [], students: [], error: new Error("Auth unavailable") };
    if (!user) return { user: null, educatorProfile: null, entitlements: [], classes: [], students: [], error: null };
    if (user.is_anonymous) return { user, anonymous: true, educatorProfile: null, entitlements: [], classes: [], students: [], error: null };
    const [profile, entitlements, classes, students, memberships] = await Promise.all([
    ...
    const queryError = profile.error || entitlements.error || classes.error || students.error || memberships.error;
    if (queryError) {
      await auth.handleSessionError(queryError);
      return { user, educatorProfile: profile.data || null, entitlements: [], classes: [], students: [], error: queryError };
    }
    const now = Date.now();
    ...
    return { user, educatorProfile: profile.data || null, entitlements: entitlements.data.filter(row => Date.parse(row.starts_at) <= now && Date.parse(row.expires_at) > now), classes: classes.data, students: studentRows, error: null };
  }
