(function () {
  ...
  function renderSignedOut(messageText = "") {
    ...
    emailCodeVerifyForm.addEventListener("submit", async event => {
      ...
      if (token.length < 6 || token.length > 10) { message.textContent = "Enter the code from your email."; return; }
  ...
  function renderClasses(snapshot) {
    ...
    return `<section class="management-section" aria-labelledby="classesHeading"><div class="section-heading"><div><h3 id="classesHeading">My Classes</h3><p>Create a class and share its Class Code with your students.</p></div></div><div class="management-grid">${classCards}</div><details class="create-panel"><summary class="button button-secondary">Create class</summary><form id="createClassForm" class="compact-form"><label for="className">Made-up class label</label><p id="classNamePrivacy" class="student-privacy-note">Use a neutral label such as Group A. Do not include student, school, or teacher names. See the privacy list in My Students below.</p><input id="className" type="text" maxlength="120" autocomplete="off" aria-describedby="classNamePrivacy" placeholder="Example: Group A" required><button class="button button-primary" type="submit">Create class</button><p id="createClassMessage" class="form-message" role="status" aria-live="polite"></p></form></details></section>`;
  }
  ...
  function renderStudentPrivacyGuide() {
    return `<aside class="student-privacy-guide" aria-labelledby="studentPrivacyHeading"><h4 id="studentPrivacyHeading">Before you create student labels or sign-in codes</h4><p><strong>Use made-up labels:</strong> Owl 7, Blue Kite, or Group A. First Volo generates the sign-in codes separately.</p><details open><summary>Information you must not enter</summary><ul><li><strong>Names:</strong> real student names, initials, or names of parents and family members.</li><li><strong>Personal identifiers:</strong> school student IDs, email addresses, phone numbers, home addresses, or birth dates.</li><li><strong>Identifying combinations:</strong> school or teacher names combined with a grade, class, initials, or other details that could reveal the student.</li><li><strong>Recognizable personal details:</strong> real family events, medical details, or unusual circumstances that could identify a student. Keep stories fictional.</li><li><strong>Identifying media:</strong> student photos, videos, or voice recordings in any upload or saved material.</li></ul></details><p>These instructions apply to student labels, class names, stories, and notes. Keep any list matching labels to real students securely outside First Volo. Share sign-in codes only with the intended student and authorized adults.</p><p><strong>Codes do not make records anonymous.</strong> Saved learning records still need privacy protections. <a href="privacy-status.html">Read our privacy progress and remaining steps →</a></p></aside>`;
  }
